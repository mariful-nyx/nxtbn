...
class TestOrderCreateWithDiscountPercetageRate(BaseTestCase): # percentage discount
    def setUp(self):
        super().setUp()
...
class TestOrderCreateWithDiscountFixedDiscount(BaseTestCase): # Fixed discount
    def setUp(self):
        super().setUp()
        self.client = APIClient()
        self.client.login(email='test@example.com', password='testpass')
        ...
        self.test_cases = {
            '2': { # USD, EUR, BDT etc.
                'input_weight_min': 0,
                'input_weight_max': 5,  # 5kg
                'order_discount_code': 'DISCOUNT100FIXED',
    ...
    def test_order_discount_calculation_fixed(self):
        ...
        total_discount = Decimal(self.discount.value) # fixed discount
